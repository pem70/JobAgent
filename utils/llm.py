...
def call_kimi(prompt: str, system_prompt: str = "", temperature: float = 0.3) -> str:
    """
...
def call_kimi_rerank(profile_summary: str, jobs_text: str) -> list[dict[str, Any]]:
    ...
    for attempt in range(2):
        raw = call_kimi(prompt=prompt, temperature=0.2)
        try:
...
def call_kimi_extract(description: str) -> dict[str, Any]:
    ...
    raw = call_kimi(prompt=prompt, temperature=0.1)
    try:
        parsed = json.loads(raw)
